...
def crop(img):
    return img[40:190,40:460]

def extract_pipeline(img,percentile = 35):
    img = crop(img)
    ...
    return {'letras':letters,'numeros':numbers}

def read_chars_pipeline(extraction):
    infered_letters = infer_characters(extraction['letras'], 'letras', 'me')
    infered_numbers = infer_characters(extraction['numeros'], 'numeros', 'me')
    placa = ['*'] * 7
    confianca = 1.0
